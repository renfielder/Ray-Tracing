...
// ===YOUR CODE STARTS HERE===
		let ls = new LightSample();
		let center = this.to.clone().sub(this.from);
		let outer = shadingPoint.clone().sub(this.from);
		ls.position = this.from.clone();
		let cosAngle = outer.clone().dot(center) / center.length() / outer.length();
		ls.intensity = cosAngle > Math.cos(this.cutoff * Math.PI / 180) ? this.intensity.clone().multiplyScalar(Math.max(cos_angle, 0)**this.exponent) : new THREE.Color(0,0,0);
		ls.intensity.multiplyScalar(1 / outer.lengthSq());
		ls.direction = outer.clone().normalize().multiplyScalar(-1);
		return ls;
// ---YOUR CODE ENDS HERE---
	}
}
